...
# voir : https://gist.github.com/RisenID/234c2fc531a666002ff9990ce71bc73d
# Kernel Source Paths
CROWN_KERNEL_DIRECTORY=/home/moi/A528BXXSBGYC1/
TOOLS_DIRECTORY=/home/moi/build-tools

TOOLCHAINS_DIRECTORY=/home/moi/aarch64-linux-android-4.9-043
#ok: clang 11.0.2
# TOOLCHAINS_DIRECTORY2=/home/moi/clang-r383902b1
#OK des erreurs compil clang 19.0.0 #https://github.com/userariii/AOSP-clang/blob/2eacd246e3efddf92a20b0c8dde7aaed742724dc/README.md?plain=1#L5
#TOOLCHAINS_DIRECTORY2=/home/moi/clang-r530567
#NOK boot clang 20.0.0
TOOLCHAINS_DIRECTORY2=/home/moi/clang-r547379
#nok:
#TOOLCHAINS_DIRECTORY2=/home/moi/clang-r450784b
#test clang 21
#TOOLCHAINS_DIRECTORY2=/home/moi/clang-r563880
#git clone -q https://github.com/llvm/llvm-project.git -b llvmorg-19.1.7 --depth=1 --single-branch ~/clang-19.1.7
#TOOLCHAINS_DIRECTORY2=~/clang+llvm-19.1.7-aarch64-linux-gnu
#nok:
#neutron howto UPDATE https://github.com/Neutron-Toolchains/clang-build-catalogue/tree/10032024
#bash <(curl -s "https://raw.githubusercontent.com/Neutron-Toolchains/antman/main/antman") -S
#./antman -S=10032024
#./antman --patch=glibc
#TOOLCHAINS_DIRECTORY2=/home/moi/neutron-clang
...
JOBS=32
SRC_DIR=$PWD
OUT_DIR=/home/moi/A528BXXSBGYC1
MAIN_DIR=$PWD
TC_DIR=/home/moi/clang-r383902b1
#TC_DIR=TOOLCHAINS_DIRECTORY2
#TC_DIR=/home/moi/clang+llvm-19.1.7-armv7a-linux-gnueabihf
rm -rf $OUT_DIR/out
mkdir -p $SRC_DIR/out
export PATH="$TC_DIR/bin:$PATH"
MAKE_PARAMS="-j$JOBS -C $SRC_DIR O=$SRC_DIR/out \
	ARCH=arm64 CC=clang CLANG_TRIPLE=aarch64-linux-gnu- CROSS_COMPILE=aarch64-linux-gnu CROSS_COMPILE_ARM32=arm-linux-gnueabi- LD=ld.lld LLVM=1 LLVM_IAS=1"
	
#		rm -R out
		make $MAKE_PARAMS -C $PWD  clean
		make $MAKE_PARAMS -C $PWD M=$PWD clean
		make $MAKE_PARAMS -C $PWD  mrproper
		echo
		echo "cleaned."
		break
		;;
...
set -e
set -o pipefail
#export CLANG_VERSION=${CLANG_VERSION:-clang-r563880}
echo "CLANG_VERSION "$CLANG_VERSION
ver="$("$TOOLCHAINS_DIRECTORY2"/bin/clang --version | head -n1)"
...
#TC_DIR=/home/moi/clang-r383902b1
TC_DIR=$TOOLCHAINS_DIRECTORY2
#TC_DIR=/home/moi/LLVM-20.1.0-Linux-X64
#TC_DIR=TOOLCHAINS_DIRECTORY2
#TC_DIR=/home/moi/clang+llvm-19.1.7-armv7a-linux-gnueabihf
BUILD_CROSS_COMPILE=/home/moi/aarch64-linux-android-4.9-043/aarch64-linux-android/bin/
rm -rf $OUT_DIR/out
mkdir -p $SRC_DIR/out
#export PATH="$TC_DIR/bin:$PATH"
...
export CC="${TOOLCHAINS_DIRECTORY2}/bin/clang"
#export PATH="${TOOLS_DIRECTORY}/path/linux-x86:${TOOLCHAINS_DIRECTORY2}/bin:$PATH"
export PATH=$TOOLCHAINS_DIRECTORY2"/bin:$PATH"
#export HOSTCFLAGS="--sysroot=${TOOLS_DIRECTORY}/sysroot -I ${TOOLS_DIRECTORY}/sysroots/x86_64-unknown-linux-musl/include"
#export HOSTLDFLAGS="--sysroot=${TOOLS_DIRECTORY}/sysroot  -Wl,-rpath,${TOOLS_DIRECTORY}/linux-x86/lib64 -L ${TOOLS_DIRECTORY}/linux-x86/lib64 -fuse-ld=lld --rtlib=compiler-rt"
echo $HOSTCFLAGS
echo $HOSTLDFLAGS
export DTC_FLAGS="-@"
export LLVM=1
...
#MAKE_PARAMS="-j$JOBS -C $SRC_DIR O=$SRC_DIR/out CROSS_COMPILE=aarch64-linux-gnu"
MAKE_PARAMS="-j$JOBS -C $SRC_DIR O=$SRC_DIR/out"


#	
DEFCONFIG=2a52sxq_eur_open_defconfig
...
#	make $MAKE_PARAMS CC="ccache clang" kernel/sched/
#	make $MAKE_PARAMS CC="ccache clang" kernel/rcu/
#	make $MAKE_PARAMS CC="ccache clang" kernel/time/
#	make $MAKE_PARAMS CC="ccache clang" kernel/sched/cpufreq_schedutil.o
#	make $MAKE_PARAMS CC="ccache clang" kernel/sched/core.o
#	make $MAKE_PARAMS CC="ccache clang" kernel/sched/fair.o
#	make $MAKE_PARAMS CC="ccache clang" kernel/irq/
#	make $MAKE_PARAMS CC="ccache clang" drivers/samsung/
#	make $MAKE_PARAMS CC="ccache clang" drivers/devfreq/
#	make $MAKE_PARAMS CC="ccache clang" drivers/cpufreq/
#	make $MAKE_PARAMS CC="ccache clang" kernel/
#	make $MAKE_PARAMS CC="ccache clang" mm/
#	make $MAKE_PARAMS CC="ccache clang" drivers/cpuidle/governors/
#	make $MAKE_PARAMS CC="ccache clang" drivers/base/power/
#	make $MAKE_PARAMS CC="ccache clang" drivers/cpufreq/
#	make $MAKE_PARAMS CC="ccache clang" drivers/samsung/
#	make $MAKE_PARAMS CC="ccache clang" block/
#	make $MAKE_PARAMS CC="ccache clang" drivers/leds/
#	make $MAKE_PARAMS CC="ccache clang" drivers/media/
#	make $MAKE_PARAMS CC="ccache clang" techpack/dataipa/drivers/platform/msm/ipa/
#	make $MAKE_PARAMS CC="ccache clang" net/ipv4/
#	make $MAKE_PARAMS CC="ccache clang" crypto/
#	make $MAKE_PARAMS CC="ccache clang" fs/
#	make $MAKE_PARAMS CC="ccache clang" fs/erofs/#	make $MAKE_PARAMS CC="ccache clang" block
#	make $MAKE_PARAMS CC="ccache clang" lib/
#	make $MAKE_PARAMS CC="ccache clang" security/selinux/
#	make $MAKE_PARAMS CC="ccache clang" fs/f2fs/
#	make $MAKE_PARAMS CC="ccache clang" drivers/android/
#	make $MAKE_PARAMS CC="ccache clang" techpack/datarmnet/
#	make $MAKE_PARAMS CC="ccache clang" drivers/staging/
#	make $MAKE_PARAMS CC="ccache clang" drivers/staging/kshrink_lruvecd/
	echo " "
#	make $MAKE_PARAMS INSTALL_MOD_PATH=modules INSTALL_MOD_STRIP=1 modules_install
	make $MAKE_PARAMS INSTALL_MOD_PATH=modules INSTALL_MOD_STRIP="--strip-debug --keep-section=.ARM.attributes" modules_install
	echo " "
...
#		export ARCH=arm64
		...
		PATH=/usr/local/bin/ccache:${PATH} make -j$(nproc --all) -C $(pwd) O=$(pwd)/out DTC_EXT=$(pwd)/tools/dtc PROJECT_NAME=a52sxq CONFIG_SEC_A52SXQ_PROJECT=y $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC='ccache'$KERNEL_LLVM_BIN HOSTCC="ccache" AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump STRIP=llvm-strip LLVM=1 vendor/moi_a52sxq_eur_open_defconfig
...
case "$cchoice" in
	y|Y )
	RAMFS_TMP=/home/moi/mkbootimg-dir2
	EDITOR_TMP=/home/moi/Android_boot_image_editor2
	EDITOR_TMP2=/home/moi/Android_boot_image_editor
	RP_REV="SRPUE26A001"
	rm "$RAMFS_TMP"/mesa.zip
		...
		rm "$RAMFS_TMP"/zip/vendor/lib/modules/*
#		cp $(find ./out/modules/lib/modules/*/ -name '*.ko') "$RAMFS_TMP"/zip/vendor/lib/modules
...
#		find "$RAMFS_TMP"/zip/vendor/lib/modules -type f -name "*.ko" -print0 | xargs -0 -n1 "${TOOLCHAINS_DIRECTORY2}/bin/llvm-strip" --strip-all --keep-section=.ARM.attributes 
#		cp ./out/modules/lib/modules/*/modules.{alias,dep,softdep} "$RAMFS_TMP"/zip/vendor/lib/modules
#		cp ./out/modules/lib/modules/*/modules.order "$RAMFS_TMP"/zip/vendor/lib/modules/modules.load
...
#		mkdir $RAMFS_TMP/tmp/
#		cd $RAMFS_TMP/tmp/
#		cp $RAMFS_TMP/a52s/boot.img $RAMFS_TMP/tmp/boot.img
#		python3 /home/moi/android_external_avb/avbtool.py erase_footer --image "$RAMFS_TMP"/tmp/boot.img
#		../magiskboot_x86 unpack "$RAMFS_TMP"/tmp/boot.img
#		rm -f "$RAMFS_TMP"/tmp/kernel
#		cp "$CROWN_KERNEL_DIRECTORY"out/arch/arm64/boot/Image "$RAMFS_TMP"/tmp/kernel
#		../magiskboot_x86 repack "$RAMFS_TMP"/tmp/boot.img "$RAMFS_TMP"/tmp/boot_new.img
#		cp "$RAMFS_TMP"/tmp/boot_new.img "$RAMFS_TMP"/zip/mesa/eur/boot.img
######		
		
		cd $EDITOR_TMP2
		./gradlew clean
		./gradlew clear
		rm -f "$EDITOR_TMP2"/vendor_boot*
		rm -f "$EDITOR_TMP2"/*.img
		cp $RAMFS_TMP/a52s/boot.img $EDITOR_TMP2/boot.img
#		cp $RAMFS_TMP/a52s/vbmeta.img $EDITOR_TMP2/vbmeta.img
		./gradlew unpack
		rm -f $EDITOR_TMP2/build/unzip_boot/kernel
		rm -f $EDITOR_TMP2/build/unzip_boot/dtb
#		cp $RAMFS_TMP/a52s/boot.avb.json $EDITOR_TMP2/build/unzip_boot/boot.avb.json
		cp "$CROWN_KERNEL_DIRECTORY"out/arch/arm64/boot/Image $EDITOR_TMP2/build/unzip_boot/kernel
		cp "$CROWN_KERNEL_DIRECTORY"out/arch/arm64/boot/dts/vendor/qcom/yupik.dtb $EDITOR_TMP2/build/unzip_boot/dtb
		./gradlew pack

		cp "$EDITOR_TMP2"/boot.img.signed "$RAMFS_TMP"/zip/mesa/eur/boot.img
...
#		mkdir $RAMFS_TMP/tmp/
#		cd $RAMFS_TMP/tmp/
#		cp $RAMFS_TMP/a52s/vendor_boot.img $RAMFS_TMP/tmp/vendor_boot.img
#		python3 /home/moi/android_external_avb/avbtool.py erase_footer --image "$RAMFS_TMP"/tmp/vendor_boot.img
#		../magiskboot_x86 unpack -h "$RAMFS_TMP"/tmp/vendor_boot.img
#		sed '1 c\name='"$RP_REV"'' header > header_new
#		rm -f header
#		mv header_new header
#		rm -f dtb
#		cp "$CROWN_KERNEL_DIRECTORY"out/arch/arm64/boot/dts/vendor/qcom/yupik.dtb "$RAMFS_TMP"/tmp/dtb
#		cp "$RAMFS_TMP"/dtb "$RAMFS_TMP"/tmp/dtb
#		../magiskboot_x86 repack "$RAMFS_TMP"/tmp/vendor_boot.img "$RAMFS_TMP"/tmp/vendor_boot_new.img
#		cp "$RAMFS_TMP"/tmp/vendor_boot_new.img "$RAMFS_TMP"/zip/mesa/eur/vendor_boot.img

		cd $EDITOR_TMP
		./gradlew clean
		./gradlew clear
		rm -f "$EDITOR_TMP"/vendor_boot*
		rm -f "$EDITOR_TMP"/boot.img
		cp $RAMFS_TMP/a52s/vendor_boot.img $EDITOR_TMP/vendor_boot.img
		./gradlew unpack
		rm -f "$EDITOR_TMP"/build/unzip_boot/dtb*
		cp "$CROWN_KERNEL_DIRECTORY"out/arch/arm64/boot/dts/vendor/qcom/yupik.dtb "$EDITOR_TMP"/build/unzip_boot/dtb
#		cp "$RAMFS_TMP"/dtb "$EDITOR_TMP"/build/unzip_boot/dtb
#		sed -i -e 's/"product" : "SRPUE26A009",/"product" : "SRPUE26A001",/g' "build/unzip_boot/vendor_boot.json"
		sed -i -e 's/"product" : "SRPUE26A011",/"product" : "SRPUE26A001",/g' "build/unzip_boot/vendor_boot.json"
		./gradlew pack
		cp "$EDITOR_TMP"/vendor_boot.img.signed "$RAMFS_TMP"/zip/mesa/eur/vendor_boot.img
		...
		cp "$RAMFS_TMP"/mesa.zip "$ZIP_N960"MOI_SBGYF1_KSU-moi-mod12.zip
...
sed -i -e 's/CONFIG_DEFAULT_BIC=y/CONFIG_DEFAULT_HTCP=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"



# # ##modif # CONFIG_DEFAULT_TCP_CONG="westwood"
sed -i -e 's/CONFIG_DEFAULT_TCP_CONG="bic"/CONFIG_DEFAULT_TCP_CONG="htcp"/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"


# add CONFIG_LITTLE_CPU_MASK=15
if ! grep -q "CONFIG_LITTLE_CPU_MASK=15" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/CONFIG_NR_CPUS=8/CONFIG_NR_CPUS=8\
CONFIG_LITTLE_CPU_MASK=15/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

# add CONFIG_BIG_CPU_MASK=240
if ! grep -q "CONFIG_BIG_CPU_MASK=240" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/CONFIG_LITTLE_CPU_MASK=15/CONFIG_LITTLE_CPU_MASK=15\
CONFIG_BIG_CPU_MASK=240/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

# # ##modif #  CONFIG_PELT_UTIL_HALFLIFE_16 is not set
sed -i -e 's/CONFIG_PELT_UTIL_HALFLIFE_16=y/# CONFIG_PELT_UTIL_HALFLIFE_16 is not set/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"



# # ##modif #  CONFIG_PELT_UTIL_HALFLIFE_10=y
sed -i -e 's/# CONFIG_PELT_UTIL_HALFLIFE_10 is not set/CONFIG_PELT_UTIL_HALFLIFE_10=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"


#modif #  CONFIG_HZ_250 is not set
sed -i -e 's/CONFIG_HZ_250=y/# CONFIG_HZ_250 is not set/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

# #modif CONFIG_HZ=250 > CONFIG_HZ=500
sed -i -e 's/CONFIG_HZ=250/CONFIG_HZ=500/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

# modif ajout CONFIG_HZ_500=y
if  grep -q "CONFIG_HZ=500" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/CONFIG_HZ=500/CONFIG_HZ=500\
CONFIG_HZ_500=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi
# # #modif CONFIG_HZ_500 is not set > CONFIG_HZ_500=y
sed -i -e 's/# CONFIG_HZ_500 is not set/CONFIG_HZ_500=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

# modif CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTIL=y
sed -i -e 's/CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTIL=y/# CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTIL is not set/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

# add CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTILX=y
if ! grep -q "CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTILX=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/# CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTIL is not set/# CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTIL is not set\
CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTILX=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

#modif CONFIG_CPU_FREQ_GOV_SCHEDUTIL is not set
sed -i -e 's/CONFIG_CPU_FREQ_GOV_SCHEDUTIL=y/# CONFIG_CPU_FREQ_GOV_SCHEDUTIL is not set/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

#add CONFIG_CPU_FREQ_GOV_SCHEDUTILX=y
if ! grep -q "CONFIG_CPU_FREQ_GOV_SCHEDUTILX=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/# CONFIG_CPU_FREQ_GOV_SCHEDUTIL is not set/# CONFIG_CPU_FREQ_GOV_SCHEDUTIL is not set\
CONFIG_CPU_FREQ_GOV_SCHEDUTILX=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi
#--------------------------------------------------------------------------

# add CONFIG_BATTERY_SAVER=y
 if ! grep -q "CONFIG_BATTERY_SAVER=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/CONFIG_MORO_SOUND=y/CONFIG_MORO_SOUND=y\
CONFIG_BATTERY_SAVER=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi


# # # modif CONFIG_ZSWAP_SAME_PAGE_SHARING=y
sed -i -e 's/# CONFIG_ZSWAP_SAME_PAGE_SHARING is not set/CONFIG_ZSWAP_SAME_PAGE_SHARING=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"


sed -i -e 's/CONFIG_ZRAM_DEFAULT_COMP_ALGORITHM="zstd"/CONFIG_ZRAM_DEFAULT_COMP_ALGORITHM="lz4"/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

# # add CONFIG_INCREASE_MAXIMUM_SWAPPINESS=y,CONFIG_FIX_INACTIVE_RATIO=y
if ! grep -q "CONFIG_INCREASE_MAXIMUM_SWAPPINESS=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/CONFIG_ZSMALLOC_STAT=y/CONFIG_ZSMALLOC_STAT=y\
CONFIG_INCREASE_MAXIMUM_SWAPPINESS=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

#test uclamp

 if ! grep -q "CONFIG_UCLAMP_TASK=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
 then
 sed -i -e 's/CONFIG_ARCH_SUPPORTS_NUMA_BALANCING=y/CONFIG_ARCH_SUPPORTS_NUMA_BALANCING=y\
CONFIG_UCLAMP_TASK=y\
CONFIG_UCLAMP_TASK_GROUP=y\
CONFIG_UCLAMP_BUCKETS_COUNT=15/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
 fi

# # replace 128 CONFIG_VM_MAX_READAHEAD=512
sed -i -e 's/CONFIG_VM_MAX_READAHEAD=128/CONFIG_VM_MAX_READAHEAD=64/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

# # modif  # CONFIG_LOD_SEC is not set
sed -i -e 's/CONFIG_LOD_SEC=y/# CONFIG_LOD_SEC is not set/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

# # modif  CONFIG_TIMA is not set
sed -i -e 's/CONFIG_TIMA=y/# CONFIG_TIMA is not set/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

# # modif  CONFIG_TIMA_LKMAUTH is not set
sed -i -e 's/CONFIG_TIMA_LKMAUTH=y/# CONFIG_TIMA_LKMAUTH is not set/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

# # modif  CONFIG_TIMA_LKM_BLOCK is not set
sed -i -e 's/CONFIG_TIMA_LKM_BLOCK=y/# CONFIG_TIMA_LKM_BLOCK is not set/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"

# # add CONFIG_KAIR=y,CONFIG_SCHED_KAIR_GLUE=y
if ! grep -q "CONFIG_KAIR=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/CONFIG_SBITMAP=y/CONFIG_SBITMAP=y\
CONFIG_KAIR=y\
CONFIG_SCHED_KAIR_GLUE=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

#add IRQ 990 https://github.com/RaySlash/android_kernel_samsung_exynos990/
if ! grep -q "CONFIG_GENERIC_IRQ_EFFECTIVE_AFF_MASK=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/CONFIG_GENERIC_IRQ_SHOW_LEVEL=y/CONFIG_GENERIC_IRQ_SHOW_LEVEL=y\
CONFIG_GENERIC_IRQ_EFFECTIVE_AFF_MASK=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

if ! grep -q "CONFIG_GENERIC_IRQ_MULTI_HANDLER=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/CONFIG_SPARSE_IRQ=y/CONFIG_SPARSE_IRQ=y\
CONFIG_GENERIC_IRQ_MULTI_HANDLER=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

# ADD CONFIG_BOEFFLA_WL_BLOCKER
if ! grep -q "CONFIG_BOEFFLA_WL_BLOCKER=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/CONFIG_SCHED_KAIR_GLUE=y/CONFIG_SCHED_KAIR_GLUE=y\
CONFIG_BOEFFLA_WL_BLOCKER=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

if ! grep -q "CONFIG_NET_UDP_TUNNEL=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/# CONFIG_NET_UDP_TUNNEL is not set/CONFIG_NET_UDP_TUNNEL=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

if ! grep -q "# CONFIG_GENEVE is not set" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
 sed -i -e 's/CONFIG_BOEFFLA_WL_BLOCKER=y/CONFIG_BOEFFLA_WL_BLOCKER=y\
# CONFIG_GENEVE is not set\
# CONFIG_GTP is not set\
CONFIG_WIREGUARD=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi
#--------- disable temp
# ajout : CONFIG_DEVFREQ_BOOST
if ! grep -q "CONFIG_DEVFREQ_BOOST=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
 sed -i -e 's/CONFIG_ARM_EXYNOS9810_BUS_DEVFREQ=y/CONFIG_ARM_EXYNOS9810_BUS_DEVFREQ=y\
CONFIG_DEVFREQ_BOOST=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

if ! grep -q "CONFIG_DEVFREQ_INPUT_BOOST_DURATION_MS" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
...
CONFIG_DEVFREQ_EXYNOS_MIF_LIGHT_INPUT_BOOST_FREQ=676000/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

#test NOK NAD
if ! grep -q "CONFIG_SEC_NAD_HPM=y" "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
then
sed -i -e 's/# CONFIG_SEC_NAD_HPM is not set/CONFIG_SEC_NAD_HPM=y/g' "/home/moi/A528BXXSBGYC1/arch/arm64/configs/exynos9810-crownlte_defconfig"
fi

		   cd /home/moi/A528BXXSBGYC1/
		   cp Makefile.moi Makefile


		  cd /home/moi/A528BXXSBGYC1/drivers/cpufreq
...
# schedutilx
		   cd /home/moi/A528BXXSBGYC1/kernel/sched/
			 cp Makefile.moi Makefile
